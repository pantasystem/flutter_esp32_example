      ...
      setState(() {
        for (ScanResult result in results) {
          if (!devices.contains(result.device)) {
            ...
            final targetService = result.advertisementData.serviceUuids.contains(BluetoothConstants.serviceUuid);
...
class DeviceScreenState extends State<DeviceScreen> {
  ...
  Future<void> writeToCharacteristic(BluetoothCharacteristic characteristic, String value) async {
    List<int> codeUnits = [];
  ...
  void sendWifiInformation() async {
    ...
    final wifiSsidCharacteristic = service?.characteristics.firstWhereOrNull((element) {
      return element.uuid.toString() == BluetoothConstants.wifiSsidCharacteristicUuid;
    });

    // wifiのpasswordを送信(書き込む)ためのcharacteristicを取得する
    final wifiPasswordCharacteristic = service?.characteristics.firstWhereOrNull((element) {
      return element.uuid.toString() == BluetoothConstants.wifiPasswordCharacteristicUuid;
    });
            ...
            children: <Widget>[
              ...
              Container(
                ...
                child: Column(
                  ...
                  children: [
                    const Text("Wi-Fi設定", style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold),),
                    ...
                    Row(
                      ...
                      children: [
                        ElevatedButton(onPressed: sendWifiInformation, child: const Text("接続"))
                      ],
                    )
                  ],
                ),
